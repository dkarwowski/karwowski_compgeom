...
class Matrix4:
    ...
    def new_orthographic(cls, scale, aspect, near, far):
        # simplified ortho projection matrix
        self = cls()
        assert near != 0.0 and near != far
        self.a = 1/(scale * aspect)
        self.f = 1/scale
        self.k = -2/(far - near)
        self.l = -(far + near)/(far - near)
        return self
    new_orthographic = classmethod(new_orthographic)
